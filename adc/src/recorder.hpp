...
class Recorder{
	...
	public:
		...
		int start_continuous_Sampling(char* arg_device, uint32_t arg_sample_rate, char* arg_start_address, char* arg_end_address)	{
			_channel_amount = 16;
			_sample_rate = arg_sample_rate;
			unsigned int chanlist[_channel_amount];
			unsigned int convert_arg = 1e9 / arg_sample_rate;
			
			int ret;
			
			comedi_cmd *cmd = (comedi_cmd*) calloc(1, sizeof(comdei_cmd));
			_device = comedi_open(arg_device);
			...
			if(comedi_set_buffer_size(_device, 0, buffer) < 0)	{
				...
				printf("Maximum buffer size set to %iB\n", comedi_get_max_buffer_size(_device, 0));
			}
			...
			if(comedi_set_buffer_size(_device, 0, buffer) < 0)	{
				...
				printf("buffer size set to %iBytes\n", comedi_get_buffer_size(_device, 0));
			}
			...
			comedi_set_global_oor_behavior(COMDI_OOR_NUMBER);
			if((ret = comedi_get_cmd_generic_timed(_device, 0, cmd, _channel_amount, 0)) < 0)	{
				printf("comedi_get_cmd_generic_timed failed\n");
				return ret;
			}
